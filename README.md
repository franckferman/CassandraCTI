...
<p align="center">
  ...
  Collect threat intel from RSS feeds, ransomware trackers, malicious domains, CISA KEV and abuse.ch,<br>
  then route it to Teams, Discord, Telegram, Email, Signal, or a live web dashboard.
</p>
...
**CassandraCTI** (named after the Trojan prophetess condemned to warn of future dangers without being believed) is a modular, asynchronous CTI pipeline (it does the opposite: it makes sure your threat intel actually reaches the people who need it).

Built from tooling I developed for enterprise security needs, it is released as open source: composable and adaptable to any security team's workflow.

**What it does:**

- Polls threat intel sources on a schedule (RSS, ransomware trackers, malicious domains, CISA KEV, abuse.ch IOCs)
- Deduplicates with a SHA1 fingerprint and a local SQLite store, so each item is sent once
- Routes by source, tag, regex or entity name; every matching route fires
- Renders Jinja2 templates and pushes to Teams, Discord, Telegram, Email or Signal
- Optional live web dashboard and periodic LLM briefings
...
| Capability | Details |
|---|---|
| **Modular sources** | RSS/Atom feeds, the **ransomware.live** family (victims free; plus Cyber Press, SEC 8-K and Stats on its PRO tier), Red Flag Domains, **CISA KEV** vulnerabilities, **abuse.ch** IOCs (Feodo/ThreatFox/URLhaus/MalwareBazaar) |
| **Modular transports** | Microsoft Teams, Discord, Telegram, Email (SMTP), Signal, and a live web dashboard (extensible) |
...
| **Entity/company watch** | Alert when a name you care about shows up in any feed (`include_terms`, accent/case-insensitive) |
| **Resilient ransomware feed** | Multi-backend fallback chain: API PRO, then API v2, then the legacy `posts.json` dump |
| **Smart deduplication** | SHA1 event fingerprint + SQLite delivery tracking, so each item is sent once |
| **Flexible routing** | Match by source prefix, tag, regex or entity name; every matching route fires |
| **Jinja2 templates** | Full control over message formatting |
| **Env var substitution** | `${VAR_NAME}` in YAML, so secrets stay out of files |
| **Dry-run and backfill** | Preview a pipeline without sending; replay past events to a transport |
...
| `api_key` | n/a | Optional PRO key; without it the chain starts at API v2 |
...
| Key | Default | Meaning |
|---|---|---|
| `include_sources` / `include_tags` / `include_regex` | n/a | select events, exactly like a route (OR across them; none = all) |
| `transports` | n/a | connector id(s) that receive the brief (must be in `transports.use`) |
...
|---|---|
| **Source** | {{ source }} |
| **Date** | {{ raw.published if raw.published else "n/a" }} |
...
Optional Prometheus metrics (`metrics.enabled: true`), served on `http://127.0.0.1:9108/metrics`: `cassandra_cti_events_sent` (per route) and `cassandra_cti_fetch_total` (per source, ok/err). Set `metrics.host: 0.0.0.0` to expose it beyond localhost.
...
```mermaid
flowchart TD
    S["Sources<br/>RSS · ransomware.live · Red Flag Domains · CISA KEV · abuse.ch"]
    F["Filter and deduplicate<br/>deny/allow · max per source · SQLite store"]
    R["Router<br/>by source / tag / regex / entity name"]
    T["Transports<br/>Teams · Discord · Telegram · Email · Signal · Web"]
    B["LLM briefings<br/>periodic recap, after delivery"]
