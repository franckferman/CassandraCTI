...
# `describe()` returns "Name (CC)" when the code is known, else the code as-is.
...
def flag(code) -> str:
    """Regional-indicator flag emoji for a 2-letter country code ("" if invalid)."""
...
def country_name(code) -> str:
    """Full country name for a code, or the code itself when unknown/empty."""
    if not isinstance(code, str) or not code.strip():
        return ""
    return COUNTRY_NAMES.get(code.strip().upper(), code.strip())


def describe(code) -> str:
    """"Name (CC)" for a known code, the code as-is when unknown, "" when empty."""
    if not isinstance(code, str) or not code.strip():
        return ""
    c = code.strip().upper()
    name = COUNTRY_NAMES.get(c)
    return f"{name} ({c})" if name else code.strip()
