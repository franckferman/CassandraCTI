...
def add_source(kind: str = typer.Argument(..., help="rss|ransomware_live|redflag"),
               ...
               tags: Optional[str] = typer.Option(None, help="liste separee par ,"),
...
def import_feeds(file: Path = typer.Argument(..., help="Chemin vers fichier CSV (Nom,URL,Tags)"),
                 config: Path = typer.Option(None)):
...
def add_connector(id: str = typer.Option(..., "--id"),
                  ...
                  emojis: bool = typer.Option(True, help="Prefixer les titres avec des emojis"),
                  emoji_map: Optional[str] = typer.Option(None, help="JSON inline ou chemin fichier JSON"),
...
def routes_add(name: str = typer.Option(...),
               include: Optional[str] = typer.Option(None, help="ex: 'rss:' ou 'ransomware.live'"),
               include_tag: Optional[str] = typer.Option(None, help="tag unique"),
               include_regex: Optional[str] = typer.Option(None, help="regex sur titre/source"),
               transports: str = typer.Option(..., help="IDs separes par ,"),
               template: Optional[Path] = typer.Option(None, help="chemin template.j2"),
               config: Path = typer.Option(None)):
    base = default_dir()
    cfg_path = config or (base / "config.yaml")
    cfg = yload(cfg_path)
...
def doctor(kind: str = typer.Argument(..., help="connector|config"),
    ...
    if kind == "config":
        try:
            _ = load_settings(str(config or (base / "config.yaml")), str(connectors or (base / "connectors.yaml")))
...
def run(config: Path = typer.Option(None), connectors: Path = typer.Option(None), loop: bool = typer.Option(False),
        sources: Optional[str] = typer.Option(None, help="ex: 'rss:' ou 'ransomware.live'"),
        dry_run: bool = typer.Option(False),
        verbose: bool = typer.Option(False),
        since: Optional[str] = typer.Option(None, help="ISO8601 ou YYYY-MM-DD"),
        no_dedupe: bool = typer.Option(False),
        interval: int = typer.Option(300, help="Intervalle en secondes pour le mode loop")):
...
def backfill(to: str = typer.Option(..., help="transport id"),
             since: str = typer.Option(..., help="YYYY-MM-DD ou ISO"),
    ...
    asyncio.run(_bf())
    typer.echo(f"Backfill OK: {len(rows)} events vers {to}")
