...
class DiscordTransport:
    ...
    def _render(self, events: List[Event], title: str | None = None, template_text: str | None = None):
        ...
        if template_text:
            ...
            if len(events) == 1:
                ...
                lines = [f"- {e.title} - [Link]({e.url or ''})" for e in events]
                txt = "\n".join(lines)
    ...
    @retry(retry=retry_if_not_exception_type(ValueError),
           ...
           wait=wait_exponential(multiplier=1, min=1, max=60), reraise=True)
        ...
        try:
            async with self._session.post(self.webhook_url, json=payload, headers={"Content-Type": "application/json"}) as resp:
                if resp.status >= 300:
                    ...
                    raise RuntimeError(f"Discord webhook error {resp.status}: {text[:200]}")
        except aiohttp.InvalidURL:
            # Deterministic (bad config): don't retry, don't leak the webhook URL.
            raise ValueError("Invalid Discord webhook URL (check the connector config)")
